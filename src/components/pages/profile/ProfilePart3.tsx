export function ProfilePart3({ open, onClose, onConfirm }: { open: boolean; onClose: () => void; onConfirm: () => void }) {
  return (
    <div className={`delete-modal-overlay${open ? ' is-active' : ''}`} id={"delete-modal-overlay"} role={"presentation"} aria-hidden={!open} onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
