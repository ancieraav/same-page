...
export function ProfilePart1({ name, age, avatarSrc, dropdownOpen, onToggle, onLogout }: { name: string; age: number; avatarSrc: string; dropdownOpen: boolean; onToggle: () => void; onLogout: () => void }) {
  return (
    <header className={"minimal-header"}>
      ...
      <div className={"header-nav"}>
        ...
        <div className={"nav-avatar-dropdown-wrap"} id={"nav-avatar-dropdown-wrap"}>
          <button type={"button"} className={"nav-avatar-icon nav-avatar-btn"} id={"header-avatar-btn"} aria-haspopup={"true"} aria-expanded={dropdownOpen} title={"Account Menu"} onClick={onToggle}>
            {avatarSrc ? <img className={"profile-avatar-img"} id={"header-avatar-img"} src={avatarSrc} alt={"Avatar"} /> : <span className={"nav-avatar-text"} id={"header-avatar-initials"}>{name.charAt(0).toUpperCase() || 'A'}</span>}
          </button>
          <div className={`avatar-dropdown-menu${dropdownOpen ? ' is-open' : ''}`} id={"avatar-dropdown-menu"} role={"menu"}>
            ...
            <button type={"button"} className={"dropdown-item dropdown-item-danger"} id={"dropdown-btn-logout"} role={"menuitem"} onClick={onLogout}>
              <svg width={"16"} height={"16"} viewBox={"0 0 24 24"} fill={"none"} stroke={"currentColor"} strokeWidth={"2"} strokeLinecap={"round"} strokeLinejoin={"round"}>
